...
- **Token Structure:** A token is composed of three parts — the expiration time, the signature and the user's email, base64url-encoded and separated by dots. See [About Tokens]({{ '/docs/tokens' | relative_url }}) for details.
...
The full API is described in the interactive [Swagger UI]({{ '/dev/api/' | absolute_url }}), with details about [Apps]({{ '/docs/apps' | relative_url }}) and [Tokens]({{ '/docs/tokens' | relative_url }}) in the rest of the documentation.
