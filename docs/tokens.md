...
Request a token for a user by sending their email to the [`POST /tokens`]({{ '/dev/api/#/tokens/post_tokens' | absolute_url }}) endpoint, authenticating with your App ID and secret headers:
...
Verify a token (for example, when the user returns through the magic link) by sending it to the [`PUT /tokens`]({{ '/dev/api/#/tokens/put_tokens' | absolute_url }}) endpoint:
